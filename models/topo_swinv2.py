...
class TopoSwin(nn.Module):
    ...
    def forward(self, x, b0, b1):
        ...
        x = self.swin.patch_embed(x)  # x is already in BHWC format

        for i, layer in enumerate(self.swin.layers):
            x = layer(x)  # x remains in BHWC format after each layer
            
            # Reshape x to (B, H*W, C) for cross-attention
            B, H, W, C = x.shape
            x_reshaped = x.view(B, H * W, C)
            
            # Project Betti encodings to match feature dimensions
            b0_proj = self.b0_proj_layers[i](b0_encoded)  # Shape: (B, seq_len, dim)
            b1_proj = self.b1_proj_layers[i](b1_encoded)
            
            # Apply cross-attention with B0 encoding
            ...
            # Apply cross-attention with B1 encoding
